...
app.get('/homepage/:id', async (req, res) => {
    ...
    const videos = await axios.get('http://localhost:5000/vidoes')
    // const data_three = await axios.get('http://localhost:5000/admin-business')
    .then(resp => resp.data.response)
...
app.post('/video/:id', async(req, res) => {
    ...
    const data = await axios.post(`http://localhost:5000/vidoes`, {...req.body})
    .then(respon => {
...
app.post('/delete-video/:videoID/:id', async(req, res) => {
    ...
    const data = await axios.post(`http://localhost:5000/delete-video/${videoID}`, {...req.body})
    .then(respon => {
        return respon.data.response })
    .catch (e => {
        console.log(e.message)
...
app.post('/recover', async(req, res) => {
    const data = await axios.post('http://localhost:5000/admin-reset-password', req.body).then(res => res.data)
...
app.post('/create-busniness/:id', async(req, res) => {
    const business = await axios.post(`http://localhost:5000/register`, {...req.body});
...
app.get('/business/:admin/:id', async (req, res) => {
    ...
    const allConsultants = await axios.get('http://localhost:5000/fetch-consultant')
    // const data_three = await axios.get('http://localhost:5000/admin-business')
    .then(resp => resp.data.consultant)
...
app.post('/update/:admin/:id', async(req, res) => {
    ...
    const data = await axios.post(`http://localhost:5000/update-business/${req.params.id}`, {...req.body})
    if(data.status == 200){
...
app.post('/suspend/:admin_id/:business_id/:type', async(req, res) => {
    ...
    if(req.params.type.trim() === 'suspend-user'){
        ...
        const data = await axios.get(`http://localhost:5000/suspend-user/${req.params.business_id}/suspend-user`)
        if(data.status === 200){
            res.redirect(`/business/${req.params.admin_id}/${req.params.business_id}`)
        }
    } else {
        const data = await axios.get(`http://localhost:5000/suspend-user/${req.params.business_id}/activate`)
        if(data.status === 200){
            res.redirect(`/business/${req.params.admin_id}/${req.params.business_id}`)
        }
    }
...
app.get('/chat/:details/:admin/:id', async (req, res) => {
    ...
    const messages = await axios.get(`http://localhost:5000/fetch-admin-message/${admin}`)
...
app.get('/consultant-chat/:consultant/:id', async (req, res) => {
    ...
    // const messages = await axios.get(`http://localhost:5000/fetch-business-messages/chat-${business._id.toString()}-${data._id}`)
